...
class App extends Component {
	...
	search( query, skipHistory ) {
		...
		let where = {};
		where[ query.attribute ] = query.searchValue;


		let search = this.api.search( query.resource, { where: where, limit: 10 } );
	...
	/**
	 * Gets the header based on the state.
	 *
	 * @returns {string} The header
	 */
	getHeader() {
		if ( this.state.searched && this.state.found ) {
			return "Found " + this.state.results.length + " result" + ( this.state.results.length !== 1 ? "s" : "" );
		}
