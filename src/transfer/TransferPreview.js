		...
		return (
			...
			<ul role="list">
				{ Object.keys( list ).map( sourceId => {
					...
					return (
						<li key={ sourceId }
						    className={ [ obj[ displayKey ] ? "" : "warning", obj.inWoo ? "" : "error" ].join( " " ) }>
							<span>{ obj[ displayKey ] || "NOT SYNCED TO MY-YOAST!" }&nbsp;</span>
							{ obj.inWoo ? <a href={ getOrderUrl( obj.sourceId, obj.sourceShopId ) } target="_blank">WooCommerce
								link</a> : <strong>( NOT FOUND IN WOOCOMMERCE! WILL NOT BE TRANSFERRED! )</strong> }
						</li>
					);
				} ) }
			</ul>
	...
	render() {
		return (
			<div className="Preview">
				<h2>Customers</h2>
				<div><strong>All data will be transferred
					from:</strong> { this.props.fromCustomer.userEmail } ( { this.props.fromCustomer.username } )
				</div>
				<div><strong>All data will be transferred
					to:</strong> { this.props.toCustomer.userEmail } ( { this.props.toCustomer.username } )
				</div>
				<h2>Data</h2>
				{ ( ! _isEmpty( this.state.orders[ 1 ] ) ) &&
				<div><strong>US Orders:</strong>{ this.getWooObjectList( this.state.orders[ 1 ], "invoiceNumber" ) }
				</div>
				}
				{ ( ! _isEmpty( this.state.orders[ 2 ] ) ) &&
				<div><strong>EU Orders:</strong>{ this.getWooObjectList( this.state.orders[ 2 ], "invoiceNumber" ) }
				</div>
				}
				{ ( ! _isEmpty( this.state.orders[ 7 ] ) ) &&
				<div><strong>UK Orders:</strong>{ this.getWooObjectList( this.state.orders[ 7 ], "invoiceNumber" ) }
				</div>
				}
				{ ( ! _isEmpty( this.state.subscriptions[ 1 ] ) ) &&
				<div><strong>US
					Subscriptions:</strong>{ this.getWooObjectList( this.state.subscriptions[ 1 ], "name" ) }</div>
				}
				{ ( ! _isEmpty( this.state.subscriptions[ 2 ] ) ) &&
				<div><strong>EU
					Subscriptions:</strong>{ this.getWooObjectList( this.state.subscriptions[ 2 ], "name" ) }</div>
				}
				{ ( ! _isEmpty( this.state.subscriptions[ 7 ] ) ) &&
				<div><strong>UK
					Subscriptions:</strong>{ this.getWooObjectList( this.state.subscriptions[ 7 ], "name" ) }</div>
				}
